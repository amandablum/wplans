...
if (!class_exists('ASP_Query')) {
    ...
    class ASP_Query {
        ...
        public function get_posts() {
            ...
            $ra['peepso_groups'] = apply_filters('asp_peepso_group_results', $ra['peepso_groups'], $args["_id"], $this);
            $rca -= count($ra['peepso_groups']);
            $results_count_adjust += $rca;

            $rca = count($ra['peepso_activities']);
            $ra['peepso_activities'] = apply_filters('asp_peepso_activities_results', $ra['peepso_activities'], $args["_id"], $this);
            $rca -= count($ra['peepso_activities']);
            $results_count_adjust += $rca;
