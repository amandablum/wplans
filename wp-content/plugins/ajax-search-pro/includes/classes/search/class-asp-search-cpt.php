    ...
    class ASP_Search_CPT extends ASP_Search {
        ...
        protected function post_process() {
            ...
            foreach ( $pageposts as $k => &$r ) {
                ...
                $r->title   = apply_filters( 'asp_result_title_before_prostproc', w_isset_def( $r->title, null ), $r->id, $searchId );
                $r->content = apply_filters( 'asp_result_content_before_prostproc', w_isset_def( $r->content, null ), $r->id, $searchId );
                $r->image   = apply_filters( 'asp_result_image_before_prostproc', w_isset_def( $r->image, null ), $r->id, $searchId );
                $r->author  = apply_filters( 'asp_result_author_before_prostproc', w_isset_def( $r->author, null ), $r->id, $searchId );
                $r->date    = apply_filters( 'asp_result_date_before_prostproc', w_isset_def( $r->date, null ), $r->id, $searchId );
        ...
        protected function adv_field( $f_args, $use_acf = false, $empty_on_missing = false ) {
            ...
            $specials = array('__title', '__content', '__link', '__url', '__image', '__date', '__author');
            ...
            if ( isset( $matches[0] ) && isset( $matches[1] ) && is_array( $matches[1] ) ) {
                foreach ( $matches[1] as $complete_field ) {
                    ...
                    if ( $field == $f_args['main_field_slug'] ) {
                        $field_pattern = str_replace( '{'.$complete_field.'}', $f_args['main_field_value'], $field_pattern );
                    } else {
